...
if($accion === "findPoblacion"){
    ...
    $stmt->execute();
      $stmt->store_result();
    $status="KO";
    $stmt->bind_result($pais,$lugar,$poblacion,$latitud,$longitud,$id);
  ...
  $result = [];
    while($stmt->fetch()){
      ...
      echo json_encode(array("status"=>"hola","result"=>$result));
...
if($accion === "obtenerPaises"){
    ...
    $paises=array();
    while($stmt->fetch()){
        if($countries[ strtoupper($pais)]!=null){
            array_push($paises,$countries[ strtoupper($pais)]);
        }
